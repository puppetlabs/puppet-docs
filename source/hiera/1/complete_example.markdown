...
[interpolation tokens]: /hiera/1/variables.html#inserting-variable-values
[hiera_lookup]: /hiera/1/puppet.html#hiera-lookup-functions
[puppet-vmwaretools]: https://github.com/craigwatson/puppet-vmwaretools
[external node classifier (ENC)]: /guides/external_nodes.html
...
[hiera.yaml]: /hiera/1/configuring.html
[Hiera command line tool]: /hiera/1/command_line.html
...
[hiera_datasources]: /hiera/1/data_sources.html
...
[hiera_include]: /hiera/1/puppet.html#assigning-classes-to-nodes-with-hiera-hierainclude
[hiera_module_data_ticket]: http://projects.puppetlabs.com/issues/16856
[automatic parameter lookup]: /hiera/1/puppet.html#automatic-parameter-lookup
[special hash and array lookups]: http://docs.puppetlabs.com/hiera/1/lookup_types.html
...
Let's get started by looking at the ntp module as it currently stands. It does all of its work in a single file, [the `init.pp` manifest][ntp_init.pp], with some ERB templates stored in its [template directory][]. So, what can we do with Hiera?
...
- We have a number of VMWare guest operating systems that we need to ensure have VMWare Tools installed.
...
* We didn't discuss how to use Hiera in module manifests, preferring to highlight its ability to provide data to parameterized classes. Hiera also provides a [collection of functions][hiera_lookup] that allow you to use Hiera within a module. Tread carefully here, though: Once you start requiring Hiera for your module to work at all, you're reducing its shareability and potentially closing it off to potential users.
