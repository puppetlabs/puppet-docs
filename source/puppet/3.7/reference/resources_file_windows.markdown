...
[mode]: /puppet/3.7/reference/type.html#file-attribute-mode

Puppet's built-in [`file`][file] resource type can manage files and directories on Windows, including ownership, group, permissions, and content. Symbolic links are supported in Puppet 3.4.0 / PE 3.2 and later on Windows 2008 / Vista and later; for details, [see the notes in the resource type reference under `file`'s `ensure` attribute](/puppet/3.7/reference/type.html#file-attribute-ensure).
...
If you set [the `source` attribute](/puppet/3.7/reference/type.html#file-attribute-source), Puppet defaults to applying the ownership and permissions that the source files have on the Puppet master server.
...
In the meantime, you can change or disable this behavior with [the `file` type's `source_permissions` attribute](/puppet/3.7/reference/type.html#file-attribute-source_permissions); for Windows systems, you will usually want to set it to `ignore` with a resource default in site.pp:
...
\*nix permissions are expressed as either an octal number or a string of symbolic modes. See [the reference for the `file` type's `mode` attribute](/puppet/3.7/reference/type.html#file-attribute-mode) for more details about the syntax.
...
* Non-`file` resource types that make partial edits to a system file (most notably the [`host`](/puppet/3.7/reference/type.html#host) resource type, which manages the `%windir%\system32\drivers\etc\hosts` file) manage their files in text mode, and will automatically translate between Windows and \*nix line endings.
