...
* [PUP-5682](https://tickets.puppetlabs.com/browse/PUP-5682): Puppet's yumrepo type now recognizes bare word `false` and the quoted string `"false"` as valid values for its boolean properties, such as gpgcheck. Previously, it only recognized `"False"` which could lead to surprising results.
