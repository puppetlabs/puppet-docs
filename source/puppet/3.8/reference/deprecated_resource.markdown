...
The [`file` type's `mode` attribute](/puppet/3.8/reference/type.html#file-attribute-mode) will accept both numbers and strings to represent numeric permissions (like `0644`).
...
The [`file` type's `recurse` attribute](/puppet/3.8/reference/type.html#file-attribute-recurse) allows a value of `inf`. (It's a synonym for `true`.)
...
The behavior of `recurse` and [`recurselimit`](/puppet/3.8/reference/type.html#file-attribute-recurselimit) used to both be crammed into the `recurse` attribute, which could be set to numbers and `inf` in addition to its current values. Now that they're handled separately, `inf` is nonsensical as a value for `recurse`.
