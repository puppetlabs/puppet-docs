...
A default value for `datadir`, used for any file-based hierarchy level that doesn’t specify its own. If not given, the data dir is the directory `data` in the same directory as the `hiera.yaml` configuration file.
...
Related topics: How custom backends work,  custom Puppet function.
...
Related topics: How hierarchies work.
...
Related topics: [interpolate][interpolation], [How hierarchies work][hierarchy].
...
* `datadir` — The directory where data files are kept. Set this only if your backend required a `:datadir` setting in its backend-specific options.
	* This path is relative to hiera.yaml’s directory: if the config file is at `/etc/puppetlabs/code/environments/production/hiera.yaml` and the datadir is set to `data`, the full path to the data directory is `/etc/puppetlabs/code/environments/production/data`. Noted that Hiera v3 uses  'hieradata' instead of 'data'.
...
Related topics: [Custom Puppet function][custom puppet function], [How custom backends work][backends], [variable interpolation][interpolation].