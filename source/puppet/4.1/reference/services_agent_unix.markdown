...
[puppetport]: /references/3.8.latest/configuration.html#puppetport
...
You can also set the `log_level` for agents in the `[agent]` section of `/etc/puppet/puppet.conf` (e.g., `log_level=debug`). 
