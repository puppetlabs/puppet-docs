...
Two bugs were discovered with the new (as of 3.6.0) `purge_ssh_keys` attribute for the [user type](/puppet/3.6/reference/type.html#user). These bugs could prevent SSH keys from being purged under certain circumstances, and have been fixed.
...
The previous default value for [`environment_timeout`](/puppet/3.6/reference/configuration.html#environmenttimeout) was 5s, which turns out to be way too short for a typical production environment. This release changes the default `environment_timeout` to 3m.
...
[allow_virtual]: /puppet/3.6/reference/type.html#package-attribute-allow_virtual
[the main manifest]: ./dirs_manifest.html
[multi_source]: /puppet/3.6/reference/type.html#file-attribute-source
...
Purging unmanaged [`ssh_authorized_key`](/puppet/3.6/reference/type.html#sshauthorizedkey) resources has been on the most-wanted features list for a very long time, and we haven't been able to make [the `resources` meta-type](/puppet/3.6/reference/type.html#resources) accommodate it.

Fortunately, the [user type](/puppet/3.6/reference/type.html#user) accommodates it very nicely. You can now purge unmanaged SSH keys for a user by setting the `purge_ssh_keys` attribute:
...
You can now change the hashing algorithm that puppet uses for file digests to `sha256` using the new [`digest_algorithm` setting](/puppet/3.6/reference/configuration.html#digestalgorithm) in puppet.conf. This is especially important for FIPS-compliant hosts, which would previously crash when puppet tried to use MD5 for hashing. Changing this setting won't affect the `md5` or `fqdn_rand` functions.
