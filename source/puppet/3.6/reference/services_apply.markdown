...
[man]: /references/3.6.latest/man/apply.html
...
[http_report]: /references/3.6.latest/report.html#http
...
To disable reporting and avoid taking up disk space with the `store` report handler, you can set [`report = false`](/references/3.6.latest/configuration.html#report) in [puppet.conf][].
