...
The `csr_attributes.yaml` file is located at `$confdir/csr_attributes.yaml` by default. (`/etc/puppetlabs/puppet/csr_attributes.yaml` in Puppet Enterprise, and `/etc/puppet/csr_attributes.yaml` in open source Puppet.)

Its location is configurable with the [`csr_attributes` setting][csr_attributes].
