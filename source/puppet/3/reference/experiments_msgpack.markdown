...
Enabling msgpack is easy, but requires a change on both the master and all agent nodes.

1. Install the [msgpack gem](http://rubygems.org/gems/msgpack) on your master and all agent nodes.
    * If you are using a Puppet Enterprise test environment, be sure to use PE's gem command instead of the system gem command. On \*nix nodes, that would be `/opt/puppet/bin/gem install msgpack`. On Windows, it would be `"C:\Program Files\Puppet Labs\Puppet\sys\ruby\bin\gem" install msgpack` on 32-bit systems; on 64-bit systems, substitute `Program Files (x86)` for `Program Files`.
