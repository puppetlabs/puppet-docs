...
You can change the priority of Puppet processes (`puppet agent`, `puppet apply`) using the [priority setting](/puppet/3.4/reference/configuration.html#priority). This can be helpful if you want to manage resource-intensive loads on busy nodes. Note that the process must be running as privileged user if it is going to *raise* its priority.
