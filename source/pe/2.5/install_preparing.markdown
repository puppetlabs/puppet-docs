...
title: "PE 2.5 » Installing » Preparing to Install"
...
You must also decide whether the puppet master server will also be filling the console role. In general, we recommend installing the master and the console on separate servers.

Plan to install PE on the puppet master **before** installing on any agent nodes. You should install the console role **immediately after** the master role if you are installing them on separate servers.
