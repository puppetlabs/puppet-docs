...
8. Provide the following information about database support (PuppetDB and the PE console databases):
   ...
   >
          ...
          exit
   > 3. If you are running PostgreSQL 9.3 or above you should install the regexp optimized index extension [`pg_trim` extension](http://www.postgresql.org/docs/9.3/static/pgtrgm.html) on the PuppetDB database. This may require the `postgresql-contrib` (or equivalent) package depending on your distribution. To install the extension, run the following commands:
   >
          sudo -u postgres sh
