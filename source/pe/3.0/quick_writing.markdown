...
Modules are stored in `/etc/puppetlabs/puppet/modules`. (This can be configured with the [`modulepath`](/puppet/3.2/reference/configuration.html#modulepath) setting in `puppet.conf`.)
...
[file_type]: /puppet/3.2/reference/type.html#file
...
* For complete documentation of the available resource types, [see the type reference](/puppet/3.2/reference/type.html).
