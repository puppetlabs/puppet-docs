...
In some cases, DNS altnames may be set up for agent nodes. In such cases, you cannot use the console to approve/reject node requests. The CSR for those nodes must be accepted or rejected using `puppet cert` on the CA. For more information, see the [DNS altnames entry in the configuration reference](/puppet/3.3/reference/configuration.html#dnsaltnames).
...
For more information on working with certificates from the CLI, see the [man page for `puppet cert`](/puppet/3.3/reference/man/cert.html).
