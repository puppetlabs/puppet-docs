...
**This page is autogenerated; any changes will get overwritten** *(last generated on Wed May 22 10:48:20 -0700 2013)*
...
: Whether to display differences when the file changes, defaulting to
  ...
  is false, then no diffs will be shown even if this parameter is true.  Valid values are `true`, `false`.
...
: Forces the mangement of local accounts when accounts are also
  being managed by some other NSS  Valid values are `true`, `false`.  Requires features libuser.
...
: The specific backend to use for this `group`
  ...
  : Group management via `groupadd` and its ilk. The default for most platforms.
    ...
    Required binaries: `groupmod`, `lgroupadd`, `groupdel`, `groupadd`.  Supported features: `system_groups`.
...
: The specific backend to use for this `service`
  ...
  : Support for AIX's System Resource controller.
    ...
    Required binaries: `/usr/sbin/rmitab`, `/usr/bin/stopsrc`, `/usr/sbin/chitab`, `/usr/bin/startsrc`, `/usr/bin/refresh`, `/usr/bin/lssrc`, `/usr/sbin/lsitab`, `/usr/sbin/mkitab`.  Default for `operatingsystem` == `aix`.  Supported features: `enableable`, `refreshable`.
  ...
  : Ubuntu service management with `upstart`.
    ...
    Required binaries: `/sbin/start`, `/sbin/status`, `/sbin/stop`, `/sbin/restart`, `/sbin/initctl`.  Default for `operatingsystem` == `ubuntu`.  Supported features: `enableable`, `refreshable`.
...
: The specific backend to use for this `user`
  ...
  : User management for AIX.
    ...
    Required binaries: `/usr/bin/chuser`, `/usr/sbin/lsgroup`, `/usr/sbin/lsuser`, `/bin/chpasswd`, `/usr/sbin/rmuser`, `/usr/bin/mkuser`.  Default for `operatingsystem` == `aix`.  Supported features: `manages_aix_lam`, `manages_expiry`, `manages_homedir`, `manages_password_age`, `manages_passwords`.
  ...
  : User and role management on Solaris, via `useradd` and `roleadd`.
    ...
    Required binaries: `usermod`, `roleadd`, `roledel`, `userdel`, `rolemod`, `passwd`, `useradd`.  Default for `osfamily` == `solaris`.  Supported features: `allows_duplicates`, `manages_homedir`, `manages_password_age`, `manages_passwords`, `manages_solaris_rbac`.
  ...
  : User management via `useradd` and its ilk.  Note that you will need to
    ...
    Required binaries: `usermod`, `luseradd`, `userdel`, `chage`, `useradd`.  Supported features: `allows_duplicates`, `manages_expiry`, `manages_homedir`, `system_users`.
...
*This page autogenerated on Wed May 22 10:48:22 -0700 2013*
