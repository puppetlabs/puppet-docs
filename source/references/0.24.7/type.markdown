...
*(last generated on Wed Dec 23 15:10:44 -0800 2009)*
...
## [Available Metaparameters](#id342)
...
If the parameter 'context' is set that that value is prepended to
PATH
...
time, nosum, mtime, md5, timestamp, md5lite. Values can also match
/\^\\{md5|md5lite|timestamp|mtime|time\\}/.
...
file. Valid values are directory, present, file, link, absent (also
called false). Values can also match /./.
...
-   **netinfo**: Host management in NetInfo.
    ...
    > currently.
    > 
    > Required binaries: nireport, niutil, mount, umount, df. Default for
...
implemented. Valid values are rule, right.
...
k-of-n. Built-in rights only show a value of '1' or absent, other
values may be acceptable. Undocumented.
...
-   **mailman**: Required binaries: list\_lists, rmlist, newlist,
...
values are 1 or 0. Default is 0. Values can also match /(0|1)/.
...
fstab and mount it. Valid values are mounted, present (also called
unmounted), absent.
...
## [nagios\_servicedependency](#id367)
...
## [nagios\_serviceescalation](#id368)
...
installed), absent, purged, latest. Values can also match /./.
...
-   **fink**: Package management via fink. Required binaries:
    /sw/bin/dpkg-query, /sw/bin/fink, /sw/bin/apt-get,
...
-   **hpux**: HP-UX's packaging system. Required binaries:
    /usr/sbin/swlist, /usr/sbin/swremove, /usr/sbin/swinstall. Default
...
-   **portage**: Provides packaging support for Gentoo's portage
    ...
    /usr/bin/update-eix. Default for operatingsystem == gentoo.
...
-   **ports**: Support for FreeBSD's ports. Again, this still mixes
    ...
    /usr/local/sbin/pkg\_deinstall, /usr/sbin/pkg\_info,
    /usr/local/sbin/portupgrade, /usr/local/sbin/portversion. Default
    for operatingsystem == freebsd. Supported features: installable,
...
true, false. Values can also match /\^\\d+$/.
...
-   **debian**: Debian's form of init-style management.
    ...
    > disabling via update-rc.d.
    > 
    > Required binaries: /usr/sbin/update-rc.d. Default for
...
-   **launchd**: launchd service management framework.
    ...
    > 
    > Required binaries: /bin/launchctl. Default for operatingsystem ==
    > darwin. Supported features: enableable, refreshable.
...
-   **smf**: Support for Sun's new Service Management Framework.
    ...
    > 
    > Required binaries: /usr/sbin/svcadm, /usr/bin/svcs. Default for
    > operatingsystem == solaris. Supported features: enableable,
    > refreshable.
...
-   **user\_role\_add**: User management inherits useradd and adds
    ...
    roleadd, userdel, usermod, roledel, useradd, rolemod. Default for
...
'absent' to remove it from the file completely Valid values are
absent. Values can also match /(0|1)/.
...
it from the file completely Valid values are absent. Values can
also match /roundrobin|priority/.
...
values are absent. Values can also match /(0|1)/.
...
'absent' to remove it from the file completely Valid values are
absent. Values can also match /[0-9]+/.
...
are absent. Values can also match /.\*/.
...
this to 'absent' to remove it from the file completely Valid values
are absent. Values can also match /[0-9]+/.
...
*This page autogenerated on Wed Dec 23 15:10:47 -0800 2009*
