...
**This page is autogenerated; any changes will get overwritten** *(last generated on 2015-11-17 19:31:08 +0000)*
...
* `lookup( as above ) |$key| { # lambda should return a default value }`
...
[data type](http://docs.puppetlabs.com/puppet/latest/reference/lang_data_type.html)
...
Finally, `lookup` can take a lambda, which should accept a single parameter.
...
default behavior.
* `'unique'` (AKA "array merge") --- Combines any number of arrays and scalar
values to return a merged, flattened array with all duplicate values removed.
The lookup will fail if any hash values are found.
...
but can adjust the merge with additional options. The `strategy` key is
required, and you can set any of these optional keys:
    * `'knockout_prefix'` (string or undef) --- A string prefix to indicate a
    value should be _removed_ from the final value. Defaults to `undef`, which
...
Do a deep hash merge lookup of user data (where the data sources can remove
items with a knockout prefix):
...
*This page autogenerated on 2015-11-17 19:31:08 +0000*
