...
sense for every executable. Each parameter has a section listed
with it in parentheses; often, that section will map to an
executable (e.g., `puppetd`), in which case it probably only makes
sense for that one executable. If `main` is listed as the section,
...
All parameters will be under a single section heading matching the
name of the process used to generate the configuraiton ('puppetd',
in this case).
...
The valid environments for Puppet clients. This is more useful as a
server-side setting than client, but any environment chosen must be
in this list. Values should be separated by a comma.
...
*This page autogenerated on Sat May 08 22:21:29 +1000 2010*
