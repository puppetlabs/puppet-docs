...
*(last generated on Wed Dec 23 15:12:55 -0800 2009)*
...
time, mtime, md5, timestamp, md5lite, nosum. Values can also match
...
file. Valid values are file, link, absent (also called false),
...
-   **netinfo**: Group management using NetInfo. Required binaries:
    niutil, nireport. Default for operatingsystem == darwin.
...
-   **netinfo**: Host management in NetInfo. This provider is highly experimental and is known
      ~ not to work currently. Required binaries: niutil, mount,
        umount, df, nireport. Default for operatingsystem == darwin.
...
'class' being a reserved word. Valid values are allow, user, deny,
rule, evaluate-mechanisms.
...
fstab and mount it. Valid values are absent, mounted, present (also
called unmounted).
...
'normal' system files. Valid values are absent, purged, present
(also called installed), latest. Values can also match (?-mix:.).
...
-   **apt**: Package management via apt-get. Required binaries:
    ...
    debian. Supported features: installable, purgeable, uninstallable,
...
-   **aptrpm**: Package management via apt-get ported to rpm.
    Required binaries: apt-get, rpm, apt-cache. Supported features:
...
-   **dpkg**: Package management via dpkg. Because this only uses dpkg
      ~ and not apt, you must specify the source of any packages you
        want to manage. Required binaries: /usr/bin/dpkg-query,
        /usr/bin/dpkg, /usr/bin/dpkg-deb. Supported features: installable,
        purgeable, uninstallable, upgradeable.
...
-   **ports**: Support for FreeBSD's ports. Again, this still mixes
    packages and ports. Required binaries: /usr/local/sbin/portversion,
    /usr/local/sbin/pkg\_deinstall, /usr/sbin/pkg\_info,
    /usr/local/sbin/portupgrade. Default for operatingsystem ==
    freebsd. Supported features: installable, uninstallable,
    upgradeable.
...
-   **rug**: Support for suse rug package manager. Required
    binaries: /usr/bin/rug, rpm. Default for operatingsystem == suse.
...
-   **debian**: Debian's form of init-style management. The only difference
      ~ is that this supports service enabling and disabling via
        ...
        operatingsystem == debian. Supported features: enableable,
        refreshable.
...
-   **redhat**: Red Hat's (and probably many others) form of init-style service
      ~ management; uses chkconfig for service enabling and disabling.
        ...
        operatingsystem == redhatfedorasusecentos. Supported features:
        enableable, refreshable.
...
The encryption type used. Usually ssh-dss or ssh-rsa for SSH
version 2. Not used for SSH version 1. Valid values are ssh-rsa
(also called rsa), ssh-dss (also called dsa).
...
-   **netinfo**: User management in NetInfo. Note that NetInfo is not smart enough to fill in default information
      ~ for users, so this provider will use default settings for home
        ...
        enable us to set a password. Required binaries: niutil, nireport.
...
*This page autogenerated on Wed Dec 23 15:12:57 -0800 2009*
