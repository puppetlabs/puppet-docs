...
*(last generated on Wed Jul 07 14:05:56 -0700 2010)*
...
-   **mailman**: Required binaries:
    `/var/lib/mailman/mail/mailman`, `list_lists`, `newlist`,
    `rmlist`.
...
-   **fink**: Package management via `fink`. Required binaries:
    ...
    `/sw/bin/apt-cache`. Supported features: `installable`,
...
-   **urpmi**: Support via `urpmi`. Required binaries: `urpmi`,
    ...
    `uninstallable`, `upgradeable`, `versionable`.
-   **yum**: Support via `yum`. Required binaries: `yum`, `python`,
    `rpm`. Default for `operatingsystem` == `fedoracentosredhat`.
...
-   **user\_role\_add**: User management inherits `useradd` and
    ...
    binaries: `roleadd`, `roledel`, `rolemod`, `userdel`, `useradd`,
...
-   **useradd**: User management via `useradd` and its ilk. Note
    ...
    binaries: `userdel`, `useradd`, `usermod`. Supported features:
...
*This page autogenerated on Wed Jul 07 14:05:58 -0700 2010*
