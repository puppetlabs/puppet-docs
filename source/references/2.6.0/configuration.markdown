...
*(last generated on Mon Jul 19 21:53:26 -0700 2010)*
...
section, and each executable will also use one of the `master`,
`agent`, or `user` sections.
...
The `puppet agent` and `puppet master` executables catch some
signals for special handling. Both daemons catch (`SIGHUP`), which
forces the server to restart tself. Predictably, interrupt and
terminate (`SIGINT` and `SIGTERM`) will shut down the server,
whether it be an instance of `puppet agent` or `puppet master`.

Sending the `SIGUSR1` signal to an instance of `puppet agent` will
cause it to immediately begin a new configuration transaction with
the server. This signal has no effect on `puppet master`.
...
instance, you'd like to pre-compile catalogs and store them in
memcached or some other easily-accessed store.
...
-   **Default**: rhizome.local

## classfile

The file in which puppet agent stores a list of the classes
associated with the retrieved configuration. Can be loaded in the
separate `puppet` executable using the `--loadclasses` option.
...
parameter is calculated based on the user. If the process is
running as root or the user that `puppet master` is supposed to run
as, it defaults to a system directory, but if it's running as any
other user, it defaults to being in `~`.
...
-   **Default**:
    [http://127.0.0.1:5984/puppet](http://127.0.0.1:5984/puppet)
...
The number of database connections. Only used when networked
...
The database password for caching. Only used when networked
databases are used.
...
The database server for caching. Only used when networked databases
are used.
...
The database user for caching. Only used when networked databases
are used.
...
`puppet agent`) this determines the environment itself, which is
used to find modules and much more. For servers (i.e.,
`puppet master`) this provides the default environment for nodes we
know nothing about.
...
-   **Default**:
    $vardir/lib/facter/${File::PATH\_SEPARATOR}$vardir/facts
...
This essentially means that you can't have any code outside of a
node, class, or definition other than in the site manifest.
...
setting might improve performance for agent -\> master
communications over slow WANs. Your puppetmaster needs to support
compression (usually by activating some settings in a reverse-proxy
in front of the puppetmaster, which rules out webrick). It is
harmless to activate this settings if your master doesn't support
...
useful for initial puppet agent runs.
...
[http://projects.puppetlabs.com/projects/puppet/wiki/LDAP\_Nodes](http://projects.puppetlabs.com/projects/puppet/wiki/LDAP_Nodes)
...
true, then by default only the `runner` server is started, which
allows remote authorized and authenticated nodes to connect and
...
Where puppet master logs. This is generally not used, since syslog
...
The effective 'run mode' of the application: master, agent, or
user.
...
The name of the application, if we are running as one. The default
is essentially $0 without the path or `.rb`.
...
This is useful for interactively running puppetd.
...
-   **Default**:
    [report@rhizome.local](mailto:report@rhizome.local)
...
See
[http://projects.puppetlabs.com/projects/puppet/wiki/Using\_Mongrel](http://projects.puppetlabs.com/projects/puppet/wiki/Using_Mongrel)
for more information.
...
Where puppet agent and puppet master store state associated with
the running configuration. In the case of puppet master, this file
...
*This page autogenerated on Mon Jul 19 21:53:26 -0700 2010*
